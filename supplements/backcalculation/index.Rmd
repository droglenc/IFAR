---
layout: page
title: "Basic Data Manipulations"
subtitle: "Backcalculation of Previous Lengths"
author: "Derek H. Ogle"
css: "/css/codebox.css"
...
# RUN BOTH MANUALLY
setwd("C:/aaaWork/Web/GitHub/IFAR/supplements/backcalculation")
source("../../rhelpers/IFARhelpers.R"); modHTML("index")
if (require(FSA)) purl2("index.Rmd",newname="backcalculation",topnotes="User must set working directory appropriately.")
...
* [Common Back-Calculation Models](#common-back-calculation-models)
* [Data Organization & Manipulation](#data-organization-manipulation)
* [Computing Back-Calculated Lengths](#computing-back-calculated-lengths)
    * [Scale-Length Relationships](#scale-length-relationships)
    * [Applying the Back-Calculation Models](#applying-the-back-calculation-models)
...
All analyses in this supplement use the West Bearskin Lake (MN) Smallmouth Bass (*Micropterus dolomieu*) data from @Weisberg1993 found in `SMBassWB.csv` ([view](https://github.com/droglenc/FSA/blob/master/data-raw/SMBassWB.csv), [download](https://raw.githubusercontent.com/droglenc/FSA/master/data-raw/SMBassWB.csv), [meta-data](http://derekogle.com/fishR/data/data-html/SMBassWB.html)).  The analysis in this supplement will focus on those fish captured in 1990.  Three variables that were constant and not used in this analysis and three measurement variables that corresponded to ages that did not exist after reducing to only fish capted in 1990 were removed to save space.
```{r echo=FALSE}
##############################################################
## This is only used to make the supplement
SMBassWB <- read.csv("https://raw.githubusercontent.com/droglenc/FSA/master/data-raw/SMBassWB.csv")
##############################################################
```
```{r eval=FALSE}
SMBassWB <- read.csv("SMBassWB.csv")
```
```{r message=FALSE}
...
\noindent Fitting these regressions is demonstrated [below](#scale-length-relationships).
...
\noindent where $c$ comes from the length of the fish at the time of scale formation, the intercept of the length-scale relationship regression (e.g., from `r eqncaps("BackCalcELS",display="cite")`), or from published "standards" for a species [@Carlander1982].  The Fraser-Lee model describes a family of lines with an intercept of $c$ that pass through the ($S_{C},L_{C}$) point (@Francis1990; `r figcaps("BackCalcEx704",display="cite")`).
...
\noindent If the length-scale relationship is linear then the expected values can be found by plugging $S_{i}$ and $S_{c}$ into `r eqncaps("BackCalcELS",display="cite")` to get
...
For efficient back-calculation, the data must also be converted to *long* or *one-measurement-per-line* format.  As demonstrated in [*Introductory Fisheries Analyses with R*](http://derekogle.com/IFAR/), wide data may be converted to long data with `gather()` from `tidyr`.  As a reminder, the arguments to `gather()` are the wide data.frame, a name for the new variable in the long format that will identify the individual (i.e., which radial measurement), a name for the new variable in the long format that will be the value for the individual (i.e., the radial measurement), and the variables in the wide format that contain the measurements.
```{r }
...
The scale-length (`r eqncaps("BackCalcESL",display="cite")`) and length-scale (`r eqncaps("BackCalcELS",display="cite")`) relationships required for all but the Dahl-Lea method are computed with the wide format data.  Thus, the wide format data.frame must contain the length of the fish (e.g., `lencap`) and the total radius of the calcified structure (e.g., `radcap`) at the time of capture.  Both linear relationships are fit with `lm()` and the coefficients should be extracted with `coef()` and saved into objects.
```{r }
lm.sl <- lm(radcap~lencap,data=wb90)
( a <- coef(lm.sl)[[1]] )
( b <- coef(lm.sl)[[2]] )
...
The $L_{i}$ estimated with a back-calculation model are most easily added to the long format data.frame.  This is largely an exercise in adding a variable to the data.frame with `mutate()` from `dplyr`.  For example, the $L_{i}$ computed with all four back-calculation models are added to `wb90r` below.
```{r }
